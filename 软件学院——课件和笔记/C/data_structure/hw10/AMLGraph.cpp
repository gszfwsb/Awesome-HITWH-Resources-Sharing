...
Status CreateGraph(AMLGraph &G)
 { /* 采用邻接多重表存储结构,构造无向图G */
   ...
   VertexType va,vb;
   EBox *p;
   printf("input vertex,edge,messgaes(Y:1,N:0): ");
   scanf("%d,%d,%d",&G.vexnum,&G.edgenum,&IncInfo);
   printf("input%dvalue(<%d chars):\n",G.vexnum,MAX_NAME);
   ...
   for(k=0;k<G.edgenum;++k) /* 构造表结点链表 */
   {
     ...
     if(IncInfo) /* 边有相关信息 */
     {
       printf("input messages related(<%d char):",MAX_INFO);
       gets(s);
       l=strlen(s);
       if(l)
       {
     ...
     while(p)
       if(p->ivex==i) /* 边的i端与该顶点有关 */
       {
         if(!p->mark) /* 只输出一次 */
         {
           ...
           p->mark=visited;
           if(p->info) /* 输出附带信息 */
             printf("related information:%s ",p->info);
         }
         p=p->ilink;
       }
       else /* 边的j端与该顶点有关 */
       {
         if(!p->mark) /* 只输出一次 */
         {
           ...
           p->mark=visited;
           if(p->info) /* 输出附带信息 */
             printf("reltated messages:%s ",p->info);
         }
         p=p->jlink;
       }
     printf("\n");
